...
public class Stream_01_Test {
    ...
    public void test_stream_filter() throws Exception {
        ...
        List<Pizza> result = pizzas.stream().filter(pi -> pi.getPrice() >= 1300).collect(Collectors.toList());
    ...
    public void test_stream_anyMatch() throws Exception {
        ...
        Boolean result1 = pizzas.stream().anyMatch(pi -> pi.getPrice() >= 1300);

        // TODO valider si au moins une pizza à un prix >= 2000
        Boolean result2 = pizzas.stream().anyMatch(pi -> pi.getPrice() >= 2000);
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream().filter(or -> or.getCustomer().getFirstname().equals("Johnny")).filter(or -> or.getPizzas().stream().anyMatch(pi -> pi.getPrice() >= 1300)).collect(Collectors.toList());
